...
<body>
    <nav class="navbar">
        ...
        <ul class="nav-links">
            <li><a href="beranda.php" class="active" aria-current="page">BERANDA</a></li>
            <li><a href="tentangkami.php" class="active" aria-current="page">TENTANG KAMI</a></li>
        </ul>

        <div class="cta-buttons">
            <a href="form_donasi.php" class="btn">DONASI</a>
            <a href="dashboard_admin.php" class="btn">ADMIN</a>
        </div>
    </nav>
    ...
    <section class="hero">
        ...
        <a href="form_donasi.php" class="btn">DONASI SEKARANG</a>
    </section>
...
<div class="program-card">
  ...
  <div class="program-content">
    ...
    <?php if (isset($_SESSION['admin'])): ?>
      <a href="edit_program.php?id=<?= $row['id'] ?>" class="btn btn-edit">Edit</a>
      <a href="hapus_program.php?id=<?= $row['id'] ?>" class="btn btn-delete" onclick="return confirm('Yakin ingin menghapus program ini?')">Hapus</a>
    <?php else: ?>
      <a href="home.php?detail=<?= $row['id'] ?>" class="btn btn-outline">Selengkapnya</a>
    <?php endif; ?>
  </div>
</div>
<?php endwhile; ?>
...
<?php if (isset($_GET['detail'])):
  $id = $_GET['detail'];
  $detail = mysqli_query($konek, "SELECT * FROM program WHERE id='$id'");
  $prog = mysqli_fetch_assoc($detail);
  if ($prog):
?>
<div style="position:fixed; top:0; left:0; width:100%; height:100%; background:#0008; display:flex; align-items:center; justify-content:center; z-index:999;">
  <div style="background:white; padding:30px; max-width:700px; width:90%; border-radius:10px; box-shadow:0 0 20px #333; position:relative;">
    <h2><?= $prog['judul'] ?></h2>
    <?php if (!empty($prog['gambar'])): ?>
      <img src="<?= $prog['gambar'] ?>" style="width:100%; max-height:300px; object-fit:cover; border-radius:8px; margin-bottom:20px;">
    <?php endif; ?>
    <p><?= nl2br($prog['deskripsi']) ?></p>
    <a href="home.php" style="position:absolute; top:10px; right:15px; text-decoration:none; color:red; font-size:20px;">&times;</a>
  </div>
</div>
<?php endif; endif; ?>
